...
class movies:
    def __init__(self):
        ...
        self.tmdb_info_link = 'http://api.themoviedb.org/3/movie/%s?api_key=%s&language=%s&append_to_response=credits,releases' % ('%s', self.tmdb_key, self.lang)
    ...
    def get(self, url, idx=True):
        try:
            ...
            elif u in self.filmesportuguesesonline_link:
                self.list = cache.get(self.filmesportuguesesonline_list, 0, url)
                print self.list
                if idx == True: self.worker()# ; self.list = [i for i in self.list if not i['poster'] == '0']
    ...
    def filmesportuguesesonline_list(self, url):
        print url
        imdbcodes = []
    ...
    def SdP_tmdb_info(self, result_imdb, next):
        ...
        if result_imdb != '0' and result_imdb != '':
            ...
            try:
                ...
                print '«««««««««««««««««««««««««««««'
                #resultado = self.abrir_url_PT(self.tmdb_info_link % result_imdb)
...
##                except: pass
                ...
                #self.list.append({'title': title, 'originaltitle': title, 'year': year, 'premiered': premiered, 'studio': '0', 'genre': '0', 'duration': '0', 'rating': rating, 'votes': votes, 'mpaa': '0', 'director': '0', 'writer': '0', 'cast': '0', 'plot': plot, 'tagline': tagline, 'name': name, 'code': result_imdb, 'imdb': result_imdb, 'tmdb': tmdb, 'tvdb': '0', 'tvrage': '0', 'poster': poster, 'banner': '0', 'fanart': fanart, 'next': next})
                #self.lista.append({'title': title, 'originaltitle': title, 'year': year, 'premiered': premiered, 'studio': '0', 'genre': '0', 'duration': '0', 'rating': rating, 'votes': votes, 'mpaa': '0', 'director': '0', 'writer': '0', 'cast': '0', 'plot': plot, 'tagline': tagline, 'name': name, 'code': result_imdb, 'imdb': result_imdb, 'tmdb': tmdb, 'tvdb': '0', 'tvrage': '0', 'poster': poster, 'banner': '0', 'fanart': fanart, 'next': next_page})
                self.lista.append({'title': title, 'originaltitle': title, 'year': year, 'premiered': premiered, 'genre': '0', 'duration': '0', 'rating': rating, 'votes': votes, 'mpaa': '0', 'plot': plot, 'imdb': result_imdb, 'tvdb': '0', 'poster': '0', 'next': next})
                print self.lista
            except:
                pass
    ...
    def worker(self, level=1):
        ...
        self.fanart_tv_headers = {'api-key': self.tmdb_key}#{'api-key': 'YTc2MGMyMTEzYTM1OTk5NzFiN2FjMWU0OWUzMTAyMGQ='.decode('base64')}
    ...
    def movieDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                cm.append((control.lang(30211).encode('utf-8'), 'RunPlugin(%s?action=movieToLibrary&name=%s&title=%s&year=%s&imdb=%s)' % (sysaddon, sysname, systitle, year, imdb)))
        ...
        try:
            ...
            url = '%s?action=moviePageP&url=%s' % (sysaddon, urllib.quote_plus(url))
