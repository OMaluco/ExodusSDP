...
class seasons:
    ...
    def tvdb_list(self, tvshowtitle, year, imdb, tvdb, season, episode, lang, limit=''):
...
class episodes:
    def __init__(self):
        ...
        self.base_link = base64.urlsafe_b64decode('aHR0cDovL21ycGlyYWN5Lm1sLw==')
        ...
        self.toppt = 'http://toppt.net/category/series/'
    ...
    def get(self):
        ...
        try:
            ...
            result=re.findall('<h2 class="title">(.*?)<div class="readmore">', result.replace('\n',''), re.DOTALL)

            for results in result:

                try: i = re.compile('imdb.com/title/(.+?)/"').findall(results)[0]
                except: i='0'

                try:
                    y = re.compile('<b>ANO: </b>(.+?)<br/>').findall(results)[0]
                    y = y.replace(' ','')
                except: y='0' 

                try:
                    tse = re.compile('title="Permalink to (.+?) [[]Season(.+?)[]] [[]Adic(.+?)Episodio[]]" rel="bookmark">').findall(results.replace('.','').replace('º',''))
                    t = tse[0][0]
                    s = tse[0][1]
                    e = str(tse[0][2])
                except:
                ...
                self.lista.append({'t':t,'y':y,'i':i,'s':s,'e':e})
        except:
            pass
        ...
        try:            
            ...
            query = self.base_link+'tvshows'
            result = client.request(query)

            result=re.findall('<div class="shortpost(.*?)<div class="rating">', result.replace('\n',''), re.DOTALL)

            for results in result:
                
                try: y = re.compile('<strong>Ano: </strong>(.+?)</li>').findall(results)[0]
                except: y = '0'

                try: t = re.compile('<li><strong>Título Original: </strong>(.+?)</li>').findall(results)[0]
                except: t = '0'
                
                try: rUrl = client.parseDOM(results, 'a', ret='href')[0]
                except: rUrl = '0'
                
                try: i = re.compile('imdb.com/title/(.+?)/"').findall(results)[0]
                except: i='0'

                items.append({'t':t,'y':y,'i':i,'u':rUrl})
        ...
        try:
            ...
            resultado = client.request(self.API_SITE+'series', headers=self.headers)
    ...
    def getMrpiracy(self, item):
        ...
        tvshowtitle = str(item['nome_ingles'].encode('utf-8'))
        year = item['ano'].encode('utf-8')
        imdb = item['IMBD'].encode('utf-8')
        season = str(item['temporadas']).encode('utf-8')
        
        self.headers['Authorization'] = 'Bearer %s' % selfAddon.getSetting('tokenMrpiracy')                
        resultado = client.request(self.API_SITE+'serie/'+str(idserie)+'/temporada/'+season, headers=self.headers)                
        acesso = self.verificar_acesso(resultado)
        if acesso == 'retry':
            self.headers['Authorization'] = 'Bearer %s' % selfAddon.getSetting('tokenMrpiracy')
            resultado = client.request(self.API_SITE+'serie/'+str(idserie)+'/temporada/'+season, headers=self.headers)
    ...
    def getRatoSeason(self, item):
        t = item['t']
        y = item['y']
        i = item['i']
        rUrl = item['u']
        
        try: s = str(len(resolversSdP.list_tvshow(rUrl, self.userRato, self.passwordRato)))
        except: s = '0'

        self.list_Rato.append({'t':t,'y':y,'i':i,'s':s,'u':rUrl})
        return self.list_Rato

    def getRatoEpisode(self, item):
        t = item['t']
        y = item['y']
        i = item['i']
        s = item['s']
        ...
        try: e = str(len(resolversSdP.rato_season_episodes(rUrl, s, self.userRato, self.passwordRato)))
        except: e = '0'
