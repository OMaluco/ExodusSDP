...
pastaLegendasSDP = os.path.join(control.dataPath,'legendasSDP')#xbmc.translatePath(selfAddon.getAddonInfo('profile')).decode("utf-8")
...
class player(xbmc.Player):
    ...
    def run(self, title, year, season, episode, imdb, tvdb, url, meta, legendas=None):
        try:
            control.sleep(200)
            ...
            # Apaga files com o conteudo das legendas
            try:
                for i in range(100):
                    if i == 0: n = ''
                    else: n = i
                    if not os.path.exists(os.path.join(pastaLegendasSDP, 'TemporarySubs'+str(n)+'.pt.srt')):
                        break
                    subtitle = os.path.join(pastaLegendasSDP, 'TemporarySubs'+str(n)+'.pt.srt')
                    file = control.deleteFile(subtitle)
                    file.close()
            except:
                pass

            if legendas != 'AUDIO PT':

                if len(str(legendas)) > 4 and '.vtt' not in legendas:
                    #print 'LEGENDAS------------1'
                    try:
                        
                        subtitle = os.path.join(pastaLegendasSDP, 'TemporarySubs.pt.srt')
	    
                        if 'mrpiracy.ml' in legendas or '.vtt' in legendas:
                            content = abrir_url(urllib.quote(legendas, safe=":/"))
                        else:
                            content = client.request(legendas)
                ...
                # Verifica se já existem links de legendas
                elif os.path.exists(os.path.join(pastaLegendasSDP, str(imdb)+'.txt')):
                    try:
                        tempImdb = os.path.join(pastaLegendasSDP, str(imdb)+'.txt')
                        file = open(tempImdb, 'r')                        
                        for link_sub in file:
                            subs.append(link_sub)
                        file.close()

                        i=0
                        subs = list(set(subs))
                        for link_sub in subs:
                            if '.vtt' not in link_sub:
                                if i == 0: n = ''
                                else: n = i
                                subtitle = os.path.join(pastaLegendasSDP, 'TemporarySubs'+str(n)+'.pt.srt')

                                if 'mrpiracy.ml' in link_sub or '.vtt' in link_sub:
                                    content = abrir_url(urllib.quote(link_sub, safe=":/"))
                                else:
                                    content = client.request(link_sub)

                                l = control.openFile(subtitle, 'w')
                                l.write(str(content))
                                l.close()
                                i=i+1
                        print subs
...
##                    break
                ...
                if not os.path.exists(os.path.join(pastaLegendasSDP, 'TemporarySubs'+str(n)+'.pt.srt')):
                    break
                subtitle = os.path.join(pastaLegendasSDP, 'TemporarySubs'+str(n)+'.pt.srt')
                file = control.openFile(subtitle, 'r')
                conteudo = file.read()
                file.close()
                if len(str(conteudo)) > 4:# and os.path.exists(os.path.join(pastaLegendasSDP, 'TemporarySubs'+str(n)+'.pt.srt')):
                    xbmc.sleep(1000)
