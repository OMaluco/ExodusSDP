...
def request(url, post=None, headers=None, mobile=False, safe=False, timeout='300'):
    try:
        ...
        result = client_genesis.request(url, cookie=cookie, post=post, headers=headers, mobile=mobile, safe=safe, timeout=timeout, output='response', error=True)

        if 'HTTP Error 503' in result[0]:
            cookie = cache.get(cloudflare, 0, u, post, headers, mobile, safe, timeout)
            result = client_genesis.request(url, cookie=cookie, post=post, headers=headers, mobile=mobile, safe=safe, timeout=timeout)
        else:
            result= result[1]
        return result
    except:
...
def cloudflare(url, post, headers, mobile, safe, timeout):
    try:
        result = client_genesis.request(url, post=post, headers=headers, mobile=mobile, safe=safe, timeout=timeout, error=True)
        ...
        cookie = client_genesis.request(query, post=post, headers=headers, mobile=mobile, safe=safe, timeout=timeout, output='cookie', error=True)
        return cookie
    except:
        pass
