...
class libmovies:
    ...
    def add(self, name, title, year, imdb, range=False):
        ...
        try:
            ...
            if self.check_setting == 'true':
                ...
                src = sources().checkSources(name, title, year, imdb, '0', '0', '0', None, None, None, '0', None)
    ...
    def strmFile(self, i):
        try:
            ...
            content = '%s?action=play&title=%s&year=%s&imdb=%s&meta=%s' % (sys.argv[0], systitle, year, imdb, None)
...
class libtvshows:
    ...
    def add(self, tvshowtitle, year, imdb, tmdb, tvdb, tvrage, range=False):
        ...
        from resources.lib.indexers import episodes#(self, tvshowtitle, year, imdb, tvdb, season=None, episode=None, idx=True)
        ...
        try: items = [{'name': i['label'], 'title': i['title'], 'year': i['year'], 'imdb': i['imdb'], 'tvdb': i['tvdb'], 'season': i['season'], 'episode': i['episode'], 'tvshowtitle': i['tvshowtitle'], 'date': i['premiered']} for i in items]
        except: items = []
        print items
...
##            print lib
            ...
            try:
                ...
                if self.check_setting == 'true':
                    if i['episode'] == '1':
                        self.block = True
                        from resources.lib.sources import sources
                        src = sources().checkSources(i['name'], i['title'], i['year'], i['imdb'], '0', i['tvdb'], '0', i['season'], i['episode'], i['tvshowtitle'], '0', i['date'])
    ...
    def strmFile(self, i):
        ...
        try:
            name, title, year, imdb, tvdb, season, episode, tvshowtitle, date = i['name'], i['title'], i['year'], i['imdb'], i['tvdb'], i['season'], i['episode'], i['tvshowtitle'], i['date']
            ...
            content = '%s?action=play&title=%s&year=%s&imdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select=' % (sys.argv[0], episodetitle, year, imdb, tvdb, season, episode, systitle, syspremiered, '', '')
    ...
    def update(self, query=None, info='true'):
        ...
        try:
            ...
            for file in episode:
                try:
                    ...
                    items.append({'tvshowtitle': tvshowtitle, 'year': year, 'imdb': imdb, 'tvdb': tvdb})
                except:
                    pass
        ...
        for item in items:
            ...
            try:
                ...
                it = [{'name': i['label'], 'title': i['title'], 'year': i['year'], 'imdb': i['imdb'], 'tvdb': i['tvdb'], 'season': i['season'], 'episode': i['episode'], 'tvshowtitle': i['tvshowtitle'], 'date': i['premiered']} for i in it]
            ...
            for i in it:
                try:
                    ...
                    if int(self.date) <= int(re.sub('[^0-9]', '', str(i['date']))):
                        from resources.lib.sources import sources
                        src = sources().checkSources(i['name'], i['title'], i['year'], i['imdb'], i['tvdb'], i['season'], i['episode'], i['tvshowtitle'], i['date'])
                        if src == False: raise Exception()
