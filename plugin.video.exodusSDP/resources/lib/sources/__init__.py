...
import os,sys,pkgutil,re,json,urllib,urlparse,random,datetime,time,xbmcgui,xbmcaddon
...
class sources:
    ...
    def play(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, meta, select):
        try:
            ...
            escolherSources = ['Internacionais','Tugas','Todas']
	    sourceEscolha = xbmcgui.Dialog().select('Procurar Sources:', escolherSources)
	    sourceEscolhida = escolherSources[sourceEscolha]
	    if sourceEscolha == -1: return 'close://'
	    
            sourcesTodas = []
            for package, name, is_pkg in pkgutil.walk_packages(__path__): sourcesTodas.append(name)

            sourcesTugaUp = ['anituga','cinematugahd','filmeshare','filmesportuguesesonline','lausse','moviefreept','movi3center','mrpiracy','ratotv','redcouch','sembilhete','toppt','tugaflix','tugafree','tugahd','vizer']
            sourcesTugaOff = ['gigatuga','lusoshare','moseon','piratatuga','tugafilmesus','tugafilmescom','tugaio','tugatorrent','warezbox','warezflix']
            sourcesTuga = []
            sourcesTuga = [i for i in sourcesTodas if str(i.replace('_mv','').replace('_tv','')) in sourcesTugaUp]
                              
            sourcesI = []
            sourcesI = [i for i in sourcesTodas if str(i.replace('_mv','').replace('_tv','')) not in sourcesTugaUp]
            sourcesInternacionais = []
            sourcesInternacionais = [i for i in sourcesI if str(i.replace('_mv','').replace('_tv','')) not in sourcesTugaOff]

            if sourceEscolhida == 'Internacionais':
                for s in sourcesInternacionais:
                    presetDict.append(str(s))
                    selfAddon.setSetting('provider.'+str(s.replace('_mv','').replace('_tv','')), 'true')
                for s in sourcesTuga:
                    selfAddon.setSetting('provider.'+str(s.replace('_mv','').replace('_tv','')), 'false')
            elif sourceEscolhida == 'Tugas':
                for s in sourcesInternacionais:
                    selfAddon.setSetting('provider.'+str(s.replace('_mv','').replace('_tv','')), 'false')
                for s in sourcesTuga:
                    presetDict.append(str(s))
                    selfAddon.setSetting('provider.'+str(s.replace('_mv','').replace('_tv','')), 'true')
            elif sourceEscolhida == 'Todas':
                for s in sourcesInternacionais:
                    presetDict.append(str(s))
                    selfAddon.setSetting('provider.'+str(s.replace('_mv','').replace('_tv','')), 'true')
                for s in sourcesTuga:
            ...
            #criar file legendas PT
            #####################################
            tempImdb = os.path.join(pastaLegendasSDP, str(imdb)+'.txt')
            if not os.path.exists(tempImdb):
            ...
            items = self.getSources(title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, sourceEscolhida, presetDict)
...
##                print i
            ...
            if i['legendas']: legendas = i['legendas']
            else: legendas = None
            
            from resources.lib.modules.player import player
            player().run(title, year, season, episode, imdb, tvdb, url, meta, legendas=legendas)
        except:
            pass
        ...
        try:
            ...
            for i in range(len(items)):
                ...
                try:
                    ...
                    try:
                        if items[i]['legendas']: legendas = items[i]['legendas']
                        else: legendas = None
                    except: legendas = None
                    
                    from resources.lib.modules.player import player
                    player().run(title, year, season, episode, imdb, tvdb, self.url, meta, legendas=legendas)
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, sourceEscolhida, presetDict=[], timeout=30):#######

        if not presetDict == []: sourceDict = [i for i in presetDict]
        ...
        if sourceEscolhida == 'Internacionais': self.sourceFile = control.providercacheFile########
        elif sourceEscolhida == 'Tugas': self.sourceFile = control.providercacheFileTuga#####
        elif sourceEscolhida == 'Todas': self.sourceFile = control.providercacheFileAll##########
    ...
    def checkSources(self, name, title, year, imdb, tmdb, tvdb, tvrage, season, episode, tvshowtitle, alter, date):
        sourceDict = []
        sourcesTodas = []
        for package, name, is_pkg in pkgutil.walk_packages(__path__): sourceDict.append(name)
##
##        for s in sourcesTodas:
##            presetDict.append(str(s))
##            selfAddon.setSetting('provider.'+str(s.replace('_mv','').replace('_tv','')), 'true')
        ...
        control.makeFile(control.dataPath)
        self.sourceFile = control.providercacheFileAll
    ...
    def clearSources(self):
        ...
        try:
            ...
            control.makeFile(control.dataPath)
            dbcon = database.connect(control.providercacheFileTuga)
            dbcur = dbcon.cursor()
            dbcur.execute("DROP TABLE IF EXISTS rel_src")
            dbcur.execute("VACUUM")
            dbcon.commit()
            ...
            control.makeFile(control.dataPath)
            dbcon = database.connect(control.providercacheFileAll)
            dbcur = dbcon.cursor()
            dbcur.execute("DROP TABLE IF EXISTS rel_src")
            dbcur.execute("VACUUM")
            dbcon.commit()
    ...
    def sourcesResolve(self, item, info=False):
        ...
        try:
            ...
            provider = item['provider'].replace(' | PT-PT','').lower()
    ...
    def getConstants(self):
        ...
        self.itemProperty = 'plugin.video.exodusSDP.container.items'

        self.metaProperty = 'plugin.video.exodusSDP.container.meta'
