...
##                except: titulo = title
            ...
            for result_url in result:
                try: result_title = client_genesis.request(result_url)
                except: result_title = ''
...
##                    if S_E in hosts.upper() and 'data-src' in hosts:
                    if hosts != '':
                        ...
                        audio_filme = ''
                        try:
                            if 'PT-PT' in host_url.upper() or 'PORTUGU' in host_url.upper(): audio_filme = ' | PT-PT'
                            else: audio_filme = ''
                        except: audio_filme = ''
                        try:
                        ...
                        sources.append({'source': host, 'quality': quality, 'provider': 'Redcouch'+audio_filme, 'url': host_url, 'direct': False, 'debridonly': False})
