...
def solvemedia(data):
    try:
        ...
        result = client_genesis.request(url[0], referer='')
...
##        url = [i for i in url if 'http://www.google.com' in i]
        print data
        if not len(url) > 0: return
        #https://www.google.com/recaptcha/api/fallback?k=6LcvzRATAAAAANfMLk1IGON6wCvZeQmk4cbxtIjK
        result = client_genesis.request(data)#(url[0])
...
def keyboard(response):
    try:
        i = os.path.join(control.dataPath,'language')
        f = control.openFile(i, 'w')
        f.write(client_genesis.request(response))
        f.close()
