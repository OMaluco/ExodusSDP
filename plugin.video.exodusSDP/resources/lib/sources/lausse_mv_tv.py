...
class source:
    def __init__(self):
        self.base_link = 'http://lausse.blogspot.pt'
...
##                except: titulo = title
            ...
            result = re.compile("<h2 class='post-title entry-title'><a href='(.+?)'>(.+?)</a></h2>").findall(result)
            
            urls = ''
            for result_url,result_title in result:
                if title.upper() in result_title.upper() and year in result_title:                               
...
##        sources = []
##        sources.append({'source': 'lausse', 'quality': 'HD', 'provider': 'lausse'+url, 'url': url, 'direct': False, 'debridonly': False})
        ...
        try:
            ...
            if 'EPISODIO' in url:
                ...
                for host_url in hosts:
                    ...
                    sources.append({'source': host, 'quality': quality, 'provider': 'Lausse'+audio_filme, 'url': host_url, 'direct': False, 'debridonly': False})
            ...
            if procura == 'FILME':
                ...
                for url in result:
                    ...
                    hosts = str(hosts)
                    hosts = re.compile('<iframe(.+?)</iframe>').findall(hosts)
